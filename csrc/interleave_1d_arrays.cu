...
__global__ void interleave_1d_arrays_kernel(const float* A, const float* B, float* output, int N) {
...
extern "C" void solve(const float* A, const float* B, float* output, int N) {
    ...
    interleave_1d_arrays_kernel<<<blocksPerGrid, threadsPerBlock>>>(A, B, output, N);
